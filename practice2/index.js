...
const filteredSignal = movingAverageFilter(signal, 5);
const filteredNoise = movingAverageFilter(noise, 5);
...
// Evaluation of win size
function evaluateWindowSize(data, windowSize) {
  // Filter signal+noise
  ...
  const mse =
    shiftedData.reduce((acc, val, index) => {
      const diff = val - filteredData[index];
      return acc + Math.pow(diff, 2);
    }, 0) / shiftedData.length;
...
for (let i = 0; i < 20; i++) {
  const windowSize0 = i;
  const mse0 = evaluateWindowSize(signalPlusNoise, windowSize0);
  console.log(`MSE for window size ${windowSize0}: ${mse0}`);
}
